    ...
    for (let i = chat.lastSeq; i < events.length; i++) {
      ...
      if (event.type === 'turn/start') {
        ...
        if (reason?.kind === 'error') {
          ...
          // 正常结束：附加模型与用量统计（与 GUI 同源的会话投影：out/in/cw/cr/ctx%）
          try {
            ...
            const bits = []
            if (usage) {
              bits.push(`out ${fmtTokens(usage.outputTokens)}`)
              bits.push(`in ${fmtTokens(usage.uncachedInputTokens)}`)
              bits.push(`cw ${fmtTokens(usage.cacheWriteTokens)}`)
              bits.push(`cr ${fmtTokens(usage.cacheReadTokens)}`)
            }
            const win = Number(pressure?.contextWindow ?? 0)
            const proj = Number(pressure?.projectedTokens ?? 0)
            if (win > 0 && proj > 0) bits.push(`ctx ${Math.round((proj / win) * 100)}%`)
            if (bits.length === 0) {
