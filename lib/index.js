...
import { WeChatClient, MessageType, MessageItemType } from 'wechat-ilink-client'
...
export class WeChatBridge {
  ...
  refreshOverride() {
    ...
    try {
      ...
      this.override = next
      this.log.info(`已热加载 override.json：allowFrom=${JSON.stringify(this.effectiveAllowFrom())}，admins=${JSON.stringify(this.effectiveAdmins())}。`)
    } catch { /* 读取失败保留旧值 */ }
  }
  ...
  async handleInbound(chat, msg) {
    ...
    if (!text) {
      if (hasMediaItem(msg)) {
        await this.reply(chat, '当前版本只支持文字和语音消息；图片、文件、视频暂未开放。')
      }
      return
    }

    // 安全铁律：纯链接不自动处理
    if (this.config.blockLinks && URL_ONLY.test(text)) {
    ...
    try {
      ...
      this.log.info(`已转发消息给 AI（联系人 ${chat.to}，${text.length} 字）。`)
    } catch (error) {
