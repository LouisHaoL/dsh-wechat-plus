...
- **群聊支持（代码就绪，平台未开放）**：开启 `groups`（默认）后处理群消息：按群维护独立会话、白名单按发送成员校验、`groupRequireMention`（默认开）仅当消息 @机器人 时响应（匹配账号 ID/本地名/"@"开头，附群消息字段诊断日志）、控制命令按发送者鉴权、回传目标为群 ID。群聊无"正在输入"能力，自动跳过。⚠️ 实测确认：当前微信 ClawBot 机器人**无法被拉入群聊**（平台未开放），功能暂不可用；平台开放后开箱即用。
