...
import { parseCron, nextCronAfter, safeSendCut, createStreamSanitizer } from './pure.js'

// 兼容导出：集成测试与外部消费者仍从插件入口导入这些纯函数
export { parseCron, nextCronAfter, safeSendCut, createStreamSanitizer }
...
export class WeChatBridge {
  ...
  async onMessage(msg) {
    ...
    // 即时反馈：消息一收到就点亮微信"正在输入…"，让用户确认已收到（而非断连）
    this.setTyping(chat, true).catch(() => {})
