...
export class WeChatBridge {
  ...
  async switchProject(chat, projectId, options = {}) {
    ...
    if (!sid) {
      ...
      if (sessions === null) {
        if (!options.silent) await this.reply(chat, `当前 DSH 版本不支持会话查询，已切到【${this.projectLabel(projectId)}】（新消息将新建会话）。${busyNote}`)
  ...
  async routeMessage(chat, text) {
    ...
    if (binding.projectId === DAILY_PROJECT_ID) {
      ...
      const ambiguous = top2 && top1.score - top2.score <= margin
