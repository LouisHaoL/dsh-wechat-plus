...
export class WeChatBridge {
  ...
  async listProjectSessions(projectPath) {
    ...
    // searchSessions 返回的 title 是事件片段而非会话标题：用 DSH 标题服务批量取真实标题
    const untitled = mapped.filter((s) => !s.title)
    if (untitled.length > 0) await this.hydrateTitles(untitled)
    for (const s of mapped) if (!s.title) s.title = '未命名会话'
    return mapped
  }

  /** 批量取会话真实标题（DSH 标题服务；失败静默）。仍无标题的会话用首条用户消息截断兜底。 */
  async hydrateTitles(sessions) {
    ...
    try {
      if (query && typeof query.readTitleSnapshots === 'function') {
        ...
        for (const r of list) {
          const id = String(r?.session?.id ?? r?.sessionId ?? '')
          const title = r?.value?.title ?? r?.title ?? ''
          const hit = sessions.find((s) => s.id === id)
          if (hit && title) hit.title = truncate(String(title), 24)
        }
      }
    } catch (error) {
      this.log.warn?.(`批量取会话标题失败（忽略）：${error?.message ?? error}`)
    }
    // 标题服务也没有的（从未生成标题的会话）：读事件流用首条用户消息兜底
    for (const s of sessions.filter((x) => !x.title)) {
      try {
