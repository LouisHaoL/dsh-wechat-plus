...
/** 测试专用：注入 TTS 模块替身。 */
export function setTtsModuleForTests(mod) {
  ttsModule = mod
}
...
export class WeChatBridge {
  ...
  async onMessage(msg) {
    ...
    const indexEntry = this.chatIndex[key]
    if (indexEntry && msg.context_token && indexEntry.contextToken !== msg.context_token) {
  ...
  async createAgentFor(chat) {
    ...
    // 持久化联系人 → 会话索引（重启后恢复上下文）
    this.chatIndex[chat.key] = { sessionId: String(sessionId), lastActive: Date.now() }
    saveChatIndex(this.stateDir, this.chatIndex)
  ...
  async tryResumeAgentFor(chat, sessionId) {
    try {
      ...
      chat.busy = false
      chat.sanitizer = createStreamSanitizer()
      this.chatIndex[chat.key] = { sessionId, lastActive: Date.now() }
      saveChatIndex(this.stateDir, this.chatIndex)
  ...
  /** 语音播报回复（TTS 生成 mp3 → 语音消息发送；失败静默降级）。 */
  async sendTts(chat, text) {
    try {
      ...
      if (!clean) return
      const { MsEdgeTTS, OUTPUT_FORMAT } = await loadTtsModule()
      const tts = new MsEdgeTTS()
      await tts.setMetadata(this.config.ttsVoice || 'zh-CN-YunxiNeural', OUTPUT_FORMAT.AUDIO_24KHZ_48KBITRATE_MONO_MP3)
      const dir = join(this.stateDir, 'tts')
      if (!existsSync(dir)) mkdirSync(dir, { recursive: true })
      const file = join(dir, `tts-${Date.now()}.mp3`)
      await tts.toFile(file, clean)
