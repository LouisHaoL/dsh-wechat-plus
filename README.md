...
- ✅ **群聊（代码就绪，待平台开放）**：群里 @机器人 即触发 AI 回复的完整实现已就绪（按群维护独立会话、白名单按成员生效、@ 匹配、诊断日志）。⚠️ 实测确认：**当前微信 ClawBot 机器人无法被拉入群聊**（平台未开放机器人入群），此功能暂不可用；平台开放后开箱即用。
- ✅ **网页抓取（可选）**：配套极简 MCP fetch 服务器（`mcp/fetch-server/`），AI 仅在用户明确要求时抓取公开网页（内置内网地址拦截、20 秒超时、3 MB 上限），无需桌面端审批
- ✅ **文件回传**：AI 把生成的图表/文档保存到 `wechat-outbox/<联系人>/` 即自动发送到你微信；更有专属工具 `wechat_send_file`（直接写内容交付）与 `wechat_send_local_file`（发工作目录内已有文件）——你问 AI 要文件时它直接生成文件发过来，而不是贴代码
...
- **杀毒软件误报**：Windows Defender 等安全软件的机器学习引擎可能把 `lib/index.js` 误判为威胁（实测出现 `Trojan:Script/ObfusScript.A!ml` 误报并隔离文件，导致插件无法加载）。本插件为纯 Node.js 源码、无任何混淆，误报率极低但非零。处理：① 在安全软件中对该文件/目录（`<仓库路径>\lib`）添加排除或"允许"；② 从隔离区还原后重启 DSH；③ 可对照 GitHub 上的文件哈希核实未被篡改（`git status` 无改动即为原始文件）。
