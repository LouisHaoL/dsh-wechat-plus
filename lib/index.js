...
export function redactText(text) {
  ...
  // 2) 密码/API key/token 赋值（key: value / key=value）
  out = out.replace(/\b(api[-_]?key|apikey|secret|password|passwd|pwd|token|access[-_]?token)\b\s*[:=]\s*\S+/gi, (m) => `${m.split(/[:=]/)[0]}=***`)
  // 3) sk- 开头的 API key
...
export class WeChatBridge {
  ...
  async allSessionRecords() {
    const query = this.getSessionQuery()
    if (!query) return null // null = 服务不可用（与空列表区分）
    if (typeof query.searchSessions === 'function') {
      // 真实 DSH：按 cwd 过滤的分页搜索（跨项目也全拉回来，最多 10 页/200 条防失控）
      const records = []
      let cursor
      for (let page = 0; page < 10 && records.length < 200; page++) {
        ...
        for (const item of items) records.push({ header: item?.header ?? item, title: item?.title ?? item?.header?.title ?? null })
  ...
  async listProjectSessions(projectPath) {
    const records = await this.allSessionRecords()
    ...
    for (const record of records) {
      ...
      mapped.push({
        id: String(id),
        title: truncate(String(record.title ?? header.title ?? '未命名会话'), 24),
  ...
  async switchProject(chat, projectId) {
    const binding = this.getBinding(chat)
    if (binding.projectId === projectId) {
      await this.reply(chat, `已在【${this.projectLabel(projectId)}】中。`)
      return
    }
    let busyNote = ''
    ...
    // 没有指针时尝试落在该项目的最近活跃会话
    if (!sid) {
      const sessions = await this.listProjectSessions(projectId === DAILY_PROJECT_ID ? dailyDir(false) : projectId)
      if (sessions === null) {
        await this.reply(chat, `当前 DSH 版本不支持会话查询，已切到【${this.projectLabel(projectId)}】（新消息将新建会话）。${busyNote}`)
  ...
  /**
   * 消息投递前的路由钩子（M2 embedding 智能路由的可插拔判断点）。
   * M1 默认直通：返回 null 表示按当前绑定指针原样投递。
   * M2 在此实现：stay 判断、日常→项目自动切换、歧义三选一、切换建议。
   * 返回 { sessionId, projectId } 可强制改投；返回 null 走默认指针。
   */
  ...
  /** 为某个联系人创建独立的 DSH Agent 会话（cwd 取当前绑定项目目录）。 */
  ...
  async handleCommand(chat, trimmed) {
    ...
    const CONTROL_COMMANDS = ['/new', '/stop', '/status', '/projects', '/project', '/sessions', '/home', '/stay', '/history']
    ...
    switch (cmd) {
      ...
      default:
        ...
        await this.reply(chat, `未知命令「${trimmed}」。可用命令：/help /projects /project /sessions /home /new /stay /history /stop /status`)
    }
  }
  ...
  async sendHistory(chat, arg = '') {
    ...
    if (!Number.isInteger(n) || n < 1 || n > 20) {
      await this.reply(chat, '用法：/history [轮数]（1~20，默认 5）。')
      return
    }
    const history = chat.history ?? []
    if (history.length === 0) {
      await this.reply(chat, '暂无本地对话记录（重启或新开会话后历史不保留在内存中）。')
      return
    }
    const recent = history.slice(-n * 2)
    const body = ['【最近对话】', ...recent.map((h) => `${h.role === 'user' ? '👤' : '🤖'} ${redactText(h.text)}`)].join('\n')
    ...
    if (parts.length > HISTORY_MAX_SEGMENTS) {
      const head = parts.slice(0, HISTORY_MAX_SEGMENTS).join('\n')
      await this.reply(chat, `${head}\n\n（内容过长，已截断至 ${HISTORY_MAX_SEGMENTS} 段，可用 /history <更小轮数> 缩小范围）`)
      return
    }
