...
import { existsSync, mkdirSync, readFileSync, writeFileSync, unlinkSync, statSync, readdirSync, renameSync } from 'node:fs'
...
import { parseCron, nextCronAfter, safeSendCut, createStreamSanitizer, createWeChatMarkdownRenderer } from './pure.js'

// 兼容导出：集成测试与外部消费者仍从插件入口导入这些纯函数
export { parseCron, nextCronAfter, safeSendCut, createStreamSanitizer, createWeChatMarkdownRenderer }
  ...
  setupAgentScope(agentCtx, chatKey) {
    ...
    try {
      ...
      systemPrompt?.section({
        name: 'wechat-outbox',
        order: 400,
        text: `[微信文件回传] 你可以把需要发送给用户的文件（图片、表格、文档等）保存到目录：${dir}。文件保存后会自动发送到用户微信，无需其他操作。`
      })
    } catch { /* 注册失败不影响主流程 */ }
  }
    ...
    for (const chat of this.chats.values()) {
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          this.log.info(`已回传文件（联系人 ${chat.to}）：${entry.name}`)
        } catch (error) {
          this.log.warn(`文件回传失败（${entry.name}）：${error?.message ?? error}`)
        }
      }
    }
  }
