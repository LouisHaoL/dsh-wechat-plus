...
• /sessions [页码/关键词] 列当前项目会话
• 切换会话两步：/sessions 后直接回复数字（如 3），或 /session <n> 直选
• /home 回到日常模式
...
const KNOWN_COMMANDS = [
  ...
  '/projects', '/project', '/sessions', '/session', '/home', '/stay', '/history',
  '/models', '/model'
]
...
export class WeChatBridge {
  ...
  async allSessionRecords(projectPath) {
    ...
    for (const term of probeTerms) {
      ...
      // 防御性检查：确保查询词不为空或仅包含空白字符
      if (!term || typeof term !== 'string' || term.trim().length === 0) {
        this.log.warn?.(`跳过空查询词：${term}`)
        continue
      }
      let cursor
      try {
  ...
  async listProjectSessions(projectPath) {
    ...
    for (const record of records) {
      ...
      // 过滤子 agent / AgentTeams 会话：parentSession 非空 = 委托子会话；
      // cwd 含 .agent-teams 段 = 小队工作区会话。只保留用户直接创建的主会话。
      if (header.parentSession != null) continue
      if (String(cwd).includes('.agent-teams')) continue
  ...
  async handleCommand(chat, trimmed) {
    // 即时反馈：①微信原生「正在输入…」状态 ②慢命令先回一条 ⏳ 文字保底（typing 展示时机不受我们控制）
    if (!chat.isGroup) this.setTyping(chat, true).catch(() => {})
    const SLOW_COMMANDS = ['/sessions', '/session', '/project', '/history', '/models']
    const cmd0 = trimmed.split(/\s+/)[0].toLowerCase()
    const isSlow = SLOW_COMMANDS.includes(cmd0)
    if (isSlow) this.reply(chat, '⏳ 查询中…').catch(() => {})
    try {
  ...
  async handleCommandInner(chat, trimmed) {
    ...
    switch (cmd) {
      ...
      case '/sessions':
        ...
        break
      case '/session': {
        // 直选：/session <n> 从当前活跃的会话菜单直接选第 n 个；无菜单或无编号则等价 /sessions
        const sn = Number(arg)
        const menu = this.activeMenu(chat)
        if (Number.isInteger(sn) && sn >= 1 && menu?.kind === 'sessions' && sn <= menu.items.length) {
          this.menus.delete(chat.key)
          const item = menu.items[sn - 1]
